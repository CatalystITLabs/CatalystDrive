...
/**
 * Due to inconsistencies in our active directory entries, searchForUser(..) needed to be modified
 * to work in the Catalyst environment.  In other environments please try using the default version
 * of this class included with the jars first.
 * 
 * Specialized LDAP authentication provider which uses Active Directory configuration conventions.
 * Also comes with LdapAuthoritiesPopulator support for external Role loading.
 * <p>
...
//    }
     ...
     * @param domain the domain name
     * @param url an LDAP url (or multiple URLs)
     * @param externalAuthoritiesPopulator - to load Collection<GrantedAuthority> from external source
     */
    public ActiveDirectoryLdapAuthenticationProvider(String domain, String url, LdapAuthoritiesPopulator externalAuthoritiesPopulator) {
    	this(domain, url);
    ...
    protected Collection<? extends GrantedAuthority> loadUserAuthorities(DirContextOperations userData, String username, String password) {
    	List<GrantedAuthority> adAuthorityList = (List<GrantedAuthority>) loadUserAuthoritiesFromMemberOf(userData, username, password);
    	if (externalAuthoritiesPopulator == null) {
    		return adAuthorityList;
    	} else {
        	List<GrantedAuthority> externalAuthoritylist = (List<GrantedAuthority>) externalAuthoritiesPopulator.getGrantedAuthorities(userData, username);
        	boolean adContains = adAuthorityList != null && adAuthorityList.size() > 0;
        	boolean extContains = externalAuthoritylist != null && externalAuthoritylist.size() > 0;
        	if (!extContains) {
                logger.debug("No values from external authorities populator.");
                if (!adContains) {
        	...
        	ArrayList<GrantedAuthority> authorities = new ArrayList<GrantedAuthority>((adContains?adAuthorityList.size():0) + externalAuthoritylist.size());
            if (adContains) {
            	authorities.addAll(adAuthorityList);
            }
            authorities.addAll(externalAuthoritylist);

            if (logger.isDebugEnabled()) {
            	for (GrantedAuthority ga: externalAuthoritylist) {
            		logger.debug("ROLE(external populator): " + ga.getAuthority());
            	}
            }
            return authorities;
    	}
    ...
    protected Collection<? extends GrantedAuthority> loadUserAuthoritiesFromMemberOf(DirContextOperations userData, String username, String password) {
    ...
    private DirContextOperations searchForUser(LdapContext ctx, String username) throws NamingException {
        ...
        // String searchFilter = "(&(objectClass=user)(userPrincipalName={0}))"; //NOSONAR
        String searchFilter = "(&(objectClass=user)(sAMAccountName={0}))";

        final String bindPrincipal = username; //createBindPrincipal(username);  //NOSONAR
